...
<head>
    ...
    <title>Forum Kelas: Laporan Kas</title>
    ...
    <div class="d-flex flex-grow-1">
        ...
        <main class="p-4 flex-grow-1">
            ...
            <nav class="navbar bg-body-tertiary">
                <form class="container" method="GET" action="{{ route('laporan.index') }}">
            ...
            <div class="container mt-4">
                <h5>Total Kas : {{ $totalKas }}</h5>
                <h5>Total Tunggakan : {{ $totalTunggakan }}</h5>
                <h5>Total Pengeluaran : {{ $totalExpenses }}</h5>
                @if ($q)
                        ...
                        <tbody>
                            @forelse ($items as $index => $item)
                                <tr>
                                    ...
                                    <td class="text-center">Rp. {{ $item->jumlah_tunggakan }}</td>
                                </tr>
                        </tbody>
                    @empty
                        <div class="col-12 text-danger text-center">
                            <p>Catatan kas gagal ditemukan.</p>
                        </div>
                @endforelse
                </table>
                @endif
            ...
            @if (!$q)
                <div class="container mx-4">
                    <table class="table table-bordered table-striped w-100 mt-4">
                        ...
                        <tbody>
                            @foreach ($penunggak as $index => $item)
                                <tr>
                                    <td class="text-center">{{ $index + 1 }}</td>
                                    <td>{{ $item->nama_lengkap }}</td>
                                    <td class="text-center">Rp. {{ $item->jumlah_tunggakan }}</td>
